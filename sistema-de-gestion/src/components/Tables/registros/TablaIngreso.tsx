        ...
        () => [
            {
                ...
                muiEditTextFieldProps: ({ row }) => ({
                    ...
                    onChange: (e) => {
                        ...
                        row.original.codigoInsumo = codigo;
                        row.original.nombre = insumo ? insumo.nombre : "";
                        row.original.marca = insumo ? insumo.marca : "";
                        row.original.categoria = insumo ? insumo.categoria : "";
                    },
                    error: !!validationErrors.codigoInsumo,
                    helperText: validationErrors.codigoInsumo,
                    onFocus: () => setValidationErrors({ ...validationErrors, codigoInsumo: undefined }),

                }),
                Cell: ({ row }) => {
                    const codigo = row.original.codigoInsumo;
                    const insumo = insumos.find((p) => p.codigo === codigo);
                    return insumo ? `${insumo.codigo}` : codigo;
                },
            },
    {
        accessorKey: "nombre",
...
const validarCamposRegistro = (registro: Partial<movimiento_insumo>) => {
    ...
    if (!registro.codigoInsumo?.trim()) errores.codigoInsumo = "Codigo insumo requerido";
